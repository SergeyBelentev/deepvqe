...
def reduce_metrics(metrics: Dict[str, float], *, device: torch.device, average: bool = True) -> Dict[str, float]:
    if not metrics:
        return {}
...
def resolve_distributed(args: argparse.Namespace) -> Tuple[bool, int, int, int, str | None]:
    env_world_size = int(os.environ.get("WORLD_SIZE", "1"))
    env_rank = int(os.environ.get("RANK", "0"))
    local_rank = int(os.environ.get("LOCAL_RANK", str(args.local_rank)))

    if args.ddp and env_world_size <= 1:
        raise RuntimeError("DDP requested, but WORLD_SIZE <= 1. Launch with torchrun --nproc_per_node=... .")

    use_ddp = env_world_size > 1
    if not use_ddp:
        return False, 0, 1, local_rank, None
    ...
    dist.init_process_group(backend=backend, init_method=args.dist_init_method)
    return True, env_rank, env_world_size, local_rank, backend
...
class RuntimeSyntheticDubDataset(Dataset[Dict[str, Tensor]]):
    def __init__(
        ...
        *,
        ...
        self.virtual_length = self.epoch_steps * self.batch_size * self.world_size
        self.seed = int(seed)
        self._epoch = 0
    ...
    def _rng(self, index: int) -> random.Random:
        ...
        return random.Random(worker_seed + self._epoch * 1_000_003 + index * 10007)
...
def save_checkpoint(path: Path, *, model: nn.Module, optimizer: torch.optim.Optimizer, scaler: Optional[GradScaler], epoch: int, global_step: int, args: argparse.Namespace) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        ...
        "args": vars(args),
        "model_cfg": asdict(unwrap_model(model).cfg),
    }
...
def load_checkpoint(
    ...
    *,
    ...
    unwrap_model(model).load_state_dict(ckpt["model"], strict=strict_model)
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--ddp", action="store_true", help="Require torchrun/DDP environment; DDP is auto-enabled when WORLD_SIZE > 1")
    p.add_argument("--dist-backend", type=str, default="auto", choices=["auto", "nccl", "gloo"], help="Distributed backend")
    p.add_argument("--dist-init-method", type=str, default="env://", help="torch.distributed init method")
    p.add_argument("--local-rank", "--local_rank", dest="local_rank", type=int, default=0, help="Local rank passed by torchrun")
...
def main() -> None:
    ...
    distributed = False
    try:
        distributed, rank, world_size, local_rank, dist_backend = resolve_distributed(args)

        set_seed(args.seed)

        device = torch.device(args.device)
        if distributed and torch.cuda.is_available() and device.type == "cuda":
        ...
        if args.index_json is not None:
            ...
            print0(f"loaded index json: {args.index_json}")
        else:
            ...
            if is_main_process():
                ...
                print0(f"saved index json: {index_json_path}")

        print0(f"ref  : {ref_stats['count']} files, {human_hours(ref_stats['hours'])}, min/mean/max = {ref_stats['min_sec']:.2f}/{ref_stats['mean_sec']:.2f}/{ref_stats['max_sec']:.2f} sec")
        print0(f"tts  : {tts_stats['count']} files, {human_hours(tts_stats['hours'])}, min/mean/max = {tts_stats['min_sec']:.2f}/{tts_stats['mean_sec']:.2f}/{tts_stats['max_sec']:.2f} sec")
        print0(f"fx backend      : {args.fx_backend} (pedalboard_available={HAS_PEDALBOARD})")
        print0(f"torchaudio      : {HAS_TORCHAUDIO}")
        ...
        base_model = DubSeparator(cfg).to(device)
        base_model.set_gradient_checkpointing(bool(args.grad_checkpoint))
        try:
            optimizer = AdamW(base_model.parameters(), lr=args.lr, weight_decay=args.weight_decay, fused=(device.type == "cuda"))
        except TypeError:
            optimizer = AdamW(base_model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        ...
        if args.resume is not None:
            ...
            print0(f"Resumed from {args.resume} at epoch={start_epoch}, global_step={global_step}")
        ...
        print0(f"distributed       : {distributed}")
        print0(f"rank/world_size   : {rank}/{world_size}")
        print0(f"dist backend      : {dist_backend}")
        print0(f"params trainable  : {sum(p.numel() for p in unwrap_model(model).parameters() if p.requires_grad):,}")
        print0(f"segment_sec       : {args.segment_sec}")
        print0(f"batch per rank    : {args.batch}")
        print0(f"global batch      : {args.batch * world_size}")
        print0(f"epoch_size steps  : {args.epoch_size}")
        print0(f"amp               : {args.amp}")
        print0(f"tf32              : {bool(args.tf32)}")
        print0(f"loss coeffs       : l1={args.loss_l1} mr_sc={args.loss_mr_sc} mr_logmag={args.loss_mr_logmag}")
        print0(f"grad checkpoint   : {bool(args.grad_checkpoint)}")
        print0(f"reset optimizer   : {bool(args.reset_optimizer)}")
        print0(f"reset scaler      : {bool(args.reset_scaler)}")
        print0(f"reset global_step : {bool(args.reset_global_step)}")
        print0(f"reset epoch       : {bool(args.reset_epoch)}")
        print0(f"strict resume     : {not bool(args.non_strict_resume)}")
        ...
        try:
                ...
                for step_idx, batch in enumerate(progress, start=1):
                    ...
                    if use_scaler:
                        ...
                        if should_log:
                            ...
                            grad_norm = grad_norm_l2(unwrap_model(model))
                        else:
                            grad_norm = float("nan")
                        ...
                        loss.backward()
                        grad_norm = grad_norm_l2(unwrap_model(model)) if should_log else float("nan")
                        optimizer.step()
                    ...
                    if should_log:
                        ...
                        step_logs = reduce_metrics(logs, device=device)
                        step_extra = reduce_metrics(extra_stats, device=device)
                        step_opt = reduce_metrics(
                            {
                        ...
                        step_stats = {**step_logs, **step_extra, **step_opt}
                        ema = update_ema_dict(ema, step_stats, decay=0.95)

                        if is_main_process():
                            progress.set_postfix({
                                "loss": f"{step_logs['loss_total']:.4f}",
                                "ema": f"{ema.get('loss_total', step_logs['loss_total']):.4f}",
                                "l1": f"{step_logs['l1']:.3f}",
                                "sc": f"{step_logs['sc']:.3f}",
                                "logm": f"{step_logs['logmag']:.3f}",
                                "gate": f"{step_extra.get('crm_gate_mean', float('nan')):.3f}",
                                "|M|": f"{step_extra.get('mask_abs_mean', float('nan')):.3f}",
                                "gn": f"{step_opt.get('grad_norm', float('nan')):.2f}",
                                "lr": f"{step_opt.get('lr', optimizer.param_groups[0]['lr']):.2e}",
                            })

                progress.close()

                elapsed = time.time() - epoch_start
                running = reduce_metrics(running, device=device)
            ...
            print0("Stopping requested. Saving interrupt checkpoint...")
            if is_main_process():
            ...
            print0("KeyboardInterrupt. Saving interrupt checkpoint...")
            if is_main_process():
                save_checkpoint(args.out_dir / "interrupt.pt", model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, global_step=global_step, args=args)
            raise
    finally:
